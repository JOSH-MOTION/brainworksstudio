...
import { categorySlug, getCategoryLabel } from '@/lib/photography-categories';
...
export default function PhotographyCategoryClient({ category }: { category: string }) {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const response = await fetch('/api/portfolio?type=photography');
        if (!response.ok) throw new Error('Failed to fetch portfolio items');
        const data: PortfolioItem[] = await response.json();
        setItems(data.filter((item) => categorySlug(item.category) === category));
      } catch (err) {
      ...
      >
        ...
        >
          <Image
            ...
            alt={`${displayCategory} Photography Hero`}
            fill
            priority
            quality={95}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <motion.h1
            className="font-serif text-3xl md:text-5xl font-bold mb-4 tracking-tight capitalize text-white"
          ...
          >
            Discover our stunning portfolio of {displayCategory.toLowerCase()} photography in Accra, Ghana.
          </motion.p>
        </div>
      </motion.section>
