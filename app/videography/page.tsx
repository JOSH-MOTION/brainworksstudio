...
export default function VideographyPortfolio({ params }: { params: { category?: string } }) {
  ...
  // Client-side redirect for missing category - only runs after mounting
  useEffect(() => {
    if (isMounted && !params?.category) {
      console.warn('No category provided, redirecting to /videography/all');
      router.push('/videography/all');
    }
  }, [isMounted, params?.category, router]);

  useEffect(() => {
    // Only fetch if we have a category and are mounted
    if (!isMounted || !params?.category) return;
    ...
    fetchItems();
  }, [category, isMounted, params?.category]);
  ...
  // Show redirect message if no category
  if (!params?.category) {
    return (
      <Layout>
        <div className="min-h-[80vh] sm:min-h-screen flex items-center justify-center bg-teal-50">
          <div className="text-center">
            ...
            <p className="mt-4 text-sm text-teal-900">Redirecting...</p>
          </div>
        </div>
      </Layout>
    );
