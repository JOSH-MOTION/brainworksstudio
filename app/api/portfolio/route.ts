...
export async function PUT(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to update portfolio item', debug: error.message }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete portfolio item', debug: error.message }, { status: 500 });
  }
}